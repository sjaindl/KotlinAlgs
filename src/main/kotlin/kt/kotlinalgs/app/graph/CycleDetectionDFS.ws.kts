...
class CycleDetection {
    fun hasCycle(graph: Graph): Boolean {
        val curCallStack: MutableSet<Int> = mutableSetOf()
        val visited: MutableSet<Int> = mutableSetOf()

        graph.vertices.forEach {
            if (hasCycleRecursive(graph, it, curCallStack, visited)) return true
        }

        return false
    }

    private fun hasCycleRecursive(
        graph: Graph,
        vertice: Vertice,
        curCallStack: MutableSet<Int>,
        visited: MutableSet<Int>
    ): Boolean {
        if (curCallStack.contains(vertice.value)) return true
        if (visited.contains(vertice.value)) return false

        visited.add(vertice.value)
        curCallStack.add(vertice.value)

        graph.neighbours(vertice).forEach {
            if (hasCycleRecursive(graph, it, curCallStack, visited)) return true
        }

        curCallStack.remove(vertice.value)
